class OMXPlayer
  ...
  def self.quit
    omx_window = WindowInfo.get_omx_info
    `xdotool windowactivate #{omx_window} && xdotool key q`
  end

  def self.increase_volume
    omx_window = WindowInfo.get_omx_info
    `xdotool windowactivate #{omx_window} && xdotool key plus`
  end

  def self.decrease_volume
    omx_window = WindowInfo.get_omx_info
    `xdotool windowactivate #{omx_window} && xdotool key minus`
  end

  def self.seek_forward
    omx_window = WindowInfo.get_omx_info
    `xdotool windowactivate #{omx_window} && xdotool key Right`
  end

  def self.seek_backward
    omx_window = WindowInfo.get_omx_info
    `xdotool windowactivate #{omx_window} && xdotool key Left`
  end
